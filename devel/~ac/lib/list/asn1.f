\ ѕарсер дл€ двоичного представлени€ ASN.1-структур.
\ X.690 Information technology - ASN.1 encoding rules:
\ Specification of Basic Encoding Rules (BER),
\ Canonical Encoding Rules (CER) and
\ Distinguished Encoding Rules (DER)

\ Ќумераци€ тегов указана в тексте самого ASN.1, стр.22
\ X.680 Information technology - Abstract Syntax
\ Notation One (ASN.1): Specification of basic
\ notation

\ —правочник OIDs: http://www.alvestrand.no/objectid/1.2.840.113549.1.9.1.html
...
\ другие, из которых в поле встречаетс€ только enum
...
\ fixme: константы ASN_* частично пересекаютс€ с ~ac/lib/win/snmp/snmp.f,
\        но не €вл€ютс€ ни windows-, ни snmp- специфичными, поэтому надо 
\        перенести всЄ сюда
...
: AsnStrDer> { a u \ u2 l z lz -- a2 u2 }
  ...
  \ а дл€ 128 и более байтов в asn.1 требуетс€ 2 или более байтов на длину
...
: AsnInt@ ( a u -- x ) \ переполнение отбрасываетс€
  0 SWAP
  0 ?DO
...
0
CELL -- asNextPart \ св€зь элементов того же уровн€
CELL -- asTag
...
CELL -- asIsMultipart \ €вл€етс€ ли составным
...
CELL -- asName     \ символическое им€, составленное из пор€дковых номеров
                   \ на каждом уровне иерархии (подобно нумерации MIME-частей в IMAP)
                   \ - дл€ поиска по именам вида "ASN.1.3.2"
CONSTANT /AsnPart
...
: Asn1ParseR { a u par prev \ a2 u2 t n as -- }
  ...
  WHILE
    ...
    prev IF
      as prev asNextPart ! \ св€зываем в пр€мом пор€дке, а не в фортовом обратном
    ELSE
      as par asParts !
    THEN
    par as asPar !
...
;
: Asn1Parse { a u \ as -- as }
  \ в "a u" на уровне корн€ может быть несколько
  \ элементов, поэтому надо создать псевдокорень со счетчиком asChilds#.
  \ ¬озвращаетс€ корень списка.
  \ ≈сли вызвавшей функции нужны свойства псевдокорн€,
