...
\ - разделители строк (если текст в результате получается многострочный) тоже {CRLF}, а не одинокие переводы строки (возвраты каретки съедало слово PARSE)
...
: StripLwsp1 ( "text" -- addr u )
  ...
  WHILE
    ...
    CharAddr 1- C@ 13 =				\ был ли возврат каретки?
    IF
