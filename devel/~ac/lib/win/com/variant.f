\ $Id$
\  онстанты, структуры, макросы и функции дл€ работы с IDispatch.
\ ѕримерный аналог библиотек ~yz/automat*.f, но без нового синтаксиса,
\ в лоб, на 10 б меньше (bin).
\ ѕриближаемс€ к будущему ~ac/lib/ns/com-xt.f
...
\ сокрашение используетс€ при конструировании списка параметров
  >BSTR VT_BSTR
;
: param@ ( variant -- ... )
\ переданные по ссылке переменные (обычный FORTH.NEGATE(VAR)) рекурсивно
\ разворачиваютс€, как если бы были переданы значени€.
\ Ёто лишает некоторых возможностей, но зато логически чище,
...
: NEWVARR ( ... params# -- varr )
\ ‘ормирование массива variant'ов в формате DISPPARAMS.
\ Ќа входе стековые пары value type и число params# таких пар.
\ Ќа выходе DISPPARAMS, который надо освобождать по FREE после использовани€.
...
\ ћногие idispatch-интерфейсы реализуют некоторые "стандартные" методы
\ с жестко заданными ID, которые сложно доставать по имени -
\ требуетс€ знать IID конкретного интерфейса, и смещение в vmt, поэтому
\ "позднее св€зывание" (дл€ неизвестного заранее интерфейса) невозможно
\ кроме как через вызов по номеру (dispid), единому дл€ "стандартных" методов.
\ Ёто, например, дает возможность заменить жесткие
\  ^ en IID_ онкретна€ оллекци€ elcol ::QueryInterface THROW
\  ^ enu en ::get__newEnum( онкретной оллекции) THROW enu
\ на
\ DISPID_NEWENUM col ID@
\ ѕримеры см. в collections.f, стандартные dispid в OAIdl.h.
...
: >SARR ( addr u -- sarr ) \ удал€ть потом по SafeArrayDestroy
