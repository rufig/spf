...
: ::QueryInterface ( ppvObject iid oid - hresult )
  ...
  SFIND IF ( ppvObject iid oid ) EXECUTE EXIT THEN \ дл€ нереализованных здесь интерфейсов
...
: IINVOKE ( ... oid addr u -- ... )
\ ¬ыполнить метод с именем addr u дл€ объекта oid
\ «десь OID - не COM-oid (указатель на указатель на VTABLE),
\ а фортовый WID:
\ это упрощенна€ реализаци€ INVOKE из ~ac/lib/ns/ns.f, без поиска по предкам.
  ROT ( addr u oid )
...
: EXECUTE-BY-ID ( ... idxt -- res ) \ реализаци€ по умолчанию
  >R
  uOID @ SpfClassWid @
  BEGIN
    DUP
