...
USER uSocketEvent \ event-сокет может быть только один per thread!
                  \ vNoneventSocket? нужно переопредел€ть, если могут быть еще обычные сокеты
USER uLastSocketRead \ когда последний раз был ReadSocket, а не сообщение
\ ≈сли таймаут сокета больше частоты поступлени€ сообщений,
\ например если поставить 500 вместо 5000 в примере ниже,
\ то таймаут MsgWFMO не сработает, поэтому в этом случае стоит провер€ть
\ сокет в обработчике сообщений тоже.
\ ƒл€ упрощени€ (отмены :) этой заботы сделан трюк с uLastSocketRead.
\   тому же теперь поток может в ответе на сообщение указать, как давно он
  ...
  BEGIN
    ...
    ELSE 0=
      ...
      ELSE \ 1=сообщени€
         ...
         \ ^ здесь ToRead2 или ioctlsocket использовать нельз€, будет 10038
         TIMEOUT @ 0<>
         IF
