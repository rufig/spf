\ јналог (сырой!) библиотеки ~ac/lib/lin/sql/sql3db.f,
\ но дл€ ODBC.
...
\ вызыва€ дл€ каждого результата функцию xt с параметрами i par ppStmt
\ в запросах Ќ≈ бинд€тс€ макроподстановки :name и $name
  ...
  WHILE
    i 1+ -> i
    i par q xt EXECUTE \ возвращает флаг продолжени€
  REPEAT
;

\ ## возвращаетс€ первое поле первой строки от query
...
\ ## возвращаетс€ первое поле первой строки от query или insert_id от query2
\ ## (т.е. условна€ вставка записи в базу)

: oreg_query { $query $query1 $query2 \ id -- id }
  SqlQ @ 0= IF 70105 THROW THEN

  $query oid_query DUP -> id \ $query там Ќ≈ освободилось
  IF
