...
: reg_query { $query $query1 $query2 \ id -- id }
  $query id_query DUP -> id \ $query там Ќ≈ освободилось
  IF
...
: (xquery) { i par ppStmt -- flag }
  ...
  ppStmt db3_cols 0 ?DO
    I ppStmt db3_colu DUP 0= ( >R 2DUP S" NULL" COMPARE 0= R> OR) 
    IF 2DROP S" &#160;" THEN
...
USER uMqueryI \ дл€ опционального использовани€ в обработчиках __*
...
: (nlbquery) { i par ppStmt \ n -- flag }
  ...
  TRUE
;
: nlbquery ( addr u -- addr2 u2 ) \ простейший вариант через зап€тую
  "" SQS !
