\ Перенос из ~ac/lib/string/regexp.f, портабельный вариант.
...
ALSO SO NEW: libpcre-0.dll
ALSO SO NEW: libpcre.so

: PcreFree ( re -- ior )

\ Используется особенность реализации функции pcre_free_substring_list,
\ которая есть просто вызов (pcre_free), и даже рекомендуется в комментариях
\ к использованию вместо pcre_free.
  1 pcre_free_substring_list DROP 0 EXIT
;
