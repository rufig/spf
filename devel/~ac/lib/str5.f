...
VECT {NOTFOUND} ' PRINT-LAST-WORD TO {NOTFOUND}
...
: {eval} ( ... s -- s ) { s \ sp base state }
  ...
  state STATE !
  sp SP@ - 
  DUP 3 CELLS = IF DROP s STR+ s DUP STRLAST @ <> IF LSTRFREE THEN EXIT THEN
  DUP 2 CELLS = IF DROP 0 <# #S #> s STR+ s EXIT THEN
  DUP 1 CELLS = IF DROP s EXIT THEN
  DROP
  S" (Error: 2020)" s STR+
  sp SP!
  s
...
: XS" ( "ccc" -- )
  XPARSE" [COMPILE] XSLITERAL  \ this should work interpretively too
  STATE @ IF _PARSED"FREE THEN
; IMMEDIATE

: " ( "ccc" -- )
  PARSE" [COMPILE] STRLITERAL  \ this should work interpretively too
...
: FFILE { f \ mem -- a u } \ прочесть весь файл и закрыть хэндл
...
;
: FFILEO { f \ mem -- a u } \ прочесть весь файл и оставить открытым
  f FILE-SIZE THROW D>S DUP _LASTFILESIZE !
  DUP CELL+ ALLOCATE THROW DUP _LASTFILE ! -> mem
  mem SWAP f READ-FILE THROW
\  f CLOSE-FILE THROW
