...
: ERRORAPI ( -> )
	DUP 0= IF hdc hwdwin ReleaseDC DROP phdc DeleteDC DROP BYE THEN ;

\ Удалить контекст окна
: DELHDC
	hdc ReleaseDC DROP ;

\ Удалить контекст в оперативной памяти
: DELPHDC
	phdc DeleteDC DROP ;

\ Удалить перо для рисования
: DELPEN
	penh DeleteObject DROP ;

\ Удалить кисть для рисования
: DELBRUSH
	brush DeleteObject DROP ;

\ Удалить буфер хранящий консоль
: DELBUF
	bufh DeleteObject DROP ;

\ Выход из программы
: BYE ( -> )
	DELPEN DELHDC  DELPHDC BYE ;

\ Открытие контекста консоли
: HDCOPEN ( -> )
	hwdwin GetDC ERRORAPI TO hdc ;

\ Открытие контекста копии консоли в памяти
: PHDCOPEN ( -> )
	hwdwin CreateCompatibleDC ERRORAPI TO phdc ;

\ Выделение оперативной памяти под буфер консоли
: NEWBUF ( -> )
	DELBUF
...
: Color ( RGB -> )
	DELPEN DUP TO color
	1 0 CreatePen TO penh
	penh phdc SelectObject DROP ;

\ Установить фон рисования
: Background ( RGB -> )
	DELBRUSH DUP TO brush CreateSolidBrush TO brush
