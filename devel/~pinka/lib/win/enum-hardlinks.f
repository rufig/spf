...
0x26 CONSTANT ERROR_HANDLE_EOF

[UNDEFINED] MAX_PATH  [IF]
260 CONSTANT MAX_PATH [THEN]

: (TREAT-HARDLINK) ( xt a-buf -- )
  DUP CELL- @ ( count-wchars ) DUP 2* SWAP RBUF UTF16>ANSI DUP IF 1- THEN ROT EXECUTE
;
...
: FOR-FILENAME-HARDLINKS ( c-addr u xt -- ) \ xt ( i*x addr u -- j*x )
  ...
  FindFirstFileNameW
  DUP -1 = IF GetLastError THROW THEN
  2R@ (TREAT-HARDLINK)
  BEGIN ( h )
    DUP R@ ( h a-buf ) 
