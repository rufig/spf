...
состояние режим-команд
состояние режим-пользовательских-команд
состояние режим-перекрывающего-lit
состояние режим-пропускаемой-команды
...
\ B - pre: 
B выполняет: GetLit RAD @ 0= IF режим-пропускаемой-команды THEN ; \ префикс пропуска допустим двух ячеек
\ C - user: 
C выполняет: режим-пользовательских-команд ; \ переключаемся в "пользовательский режим"
\ D - sys: 
D выполняет: режим-перекрывающего-lit ; \ переходим в режим где lit затирает аккумулятор
\ E - fetch ( @ ) 
E выполняет: 
...
VARIABLE осталось-пользовательских-команд
режим-пользовательских-команд
на-входе: 3 осталось-пользовательских-команд ! ; \ допустим, три команды после префикса считаются "пользовательскими"
все: символ @ H. \ типа мы в "пользовательском" режиме печатаем поступающие числа
символ @ режим-команд выполнить-один-раз \ выполняем команды точно так же как и в обычном режиме
осталось-пользовательских-команд 1-!
осталось-пользовательских-команд @ 0 <> IF режим-пользовательских-команд THEN ;
\ если пользовательские команды ещё не кончились переходить в пользовательский режим

режим-перекрывающего-lit
взять-из-таблицы режим-команд \ все команды делать точно такие же как и в обычном режиме, кроме двух:
\ D - sys: 
D выполняет: режим-команд ; \ возвращение в обычный режим
\ 3 - lit 
3 выполняет: GetLit RAD ! ; \  lit , затирающий аккумулятор

режим-пропускаемой-команды
все: режим-команд ; \ ничего не делаем, получаемый номер команды никак не обрабатываем
\ пропускаемая команда только одна, поэтому сразу после её "обработки" переходим обратно в обычный режим
...
: STEP CMD@ выполнить-один-раз advance ;

\ рабочий цикл 
: pp CLS 
  0 0 AT-XY  S" [ESC] - выход" TYPE 
  OUTMST CR
режим-команд \ ставим заранее режим
