...
divi  asc: / ;
neg   asc: NEGATE ;
fact  asc: DUP 2 < IF DROP 1 ELSE 1 SWAP 1+ 1 ?DO I * LOOP THEN ; \ факториал
...
\ итератор, перебирающий все комбанации цифр указанных в строке 
\ addr u и операций
...
<*> \ альтернатива 2: функции с одним операндом
	...
	<*>
		...
		ops top fact <> ONTRUE
		get-two NIP 13 > ONFALSE \ от чисел больших 13 факториал брать бесполезно -- не влезает в 2^32
		fact op-bpush
	<*
