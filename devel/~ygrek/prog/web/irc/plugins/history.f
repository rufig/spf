...
: !last
    ...
    2 get-group NIP IF
     ...
     THEN THEN
     * START{ secs-get-history-> counter counter_max < IF log>string message-sender S-NOTICE-TO counter 1+ TO counter ELSE DROP THEN }EMERGE
    ELSE
     START{ n-get-history-> counter counter_max < IF log>string message-sender S-NOTICE-TO counter 1+ TO counter ELSE DROP THEN }EMERGE
    THEN
  ;
