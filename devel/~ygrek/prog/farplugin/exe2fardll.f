...
\ ѕодправлено дл€ экспорта OpenPlugin SetStartupInfo GetPluginInfo
...
( надо фиксить VOC-LIST на старое значение.
  ѕотому как. даже если словарь создаетс€ во временном хранилище /во временном словаре/
  он добавл€етс€ в VOC-LIST
  =-> VOC-LIST должно быть локальным дл€ хранилища!
)
...
: Make-RelocTable ( -- a u )
  ...
  vReloc reloc-offs @
;
( значени€ самой перовой провер€емой €чейки должно быть одинаково
в обоих образах)
