...
\  онтекст рисовни€ OpenGL
\ —остоит из Device Context (GDI) и Rendering Context (GL)
...
\ GL полотно - то на чЄм собственно происходит рисование
\ требует hwnd окна на котором рисует (метод :initialize)
\ сообщени€ Ќ≈ перехватывает - это делаетс€ в классах-обЄртках
CLASS CGLCanvas

  VAR _hwnd
  CGLContext OBJ _context
  VAR _width    \ ширина 
  VAR _height   \ высота полотна дл€ рисовани€
  CGLObjectList OBJ _scene   \ список обьектов дл€ отрисовки
...
\ «апрашиваемые размеры в _width и _height
: :resize \ Resize And Initialize The GL Window
    _hwnd @ _context :enable

       \ “еперь устанавливаем GL параметры окна
