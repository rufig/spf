...
\ AUGUST 28TH, 2000 - 9:49 TJZ
\ ADDED "FDIVRP" AND "FSUBRP" FOR COMPATIBILITY WITH SOME OTHER ASSEMBLERS
\ ARM 02/12/2003 19:20:37 CORRECTION FOR <0 [EBP] [REG]>
\ ARM 20/12/2004 22:53:40 ADDED [WORD] AND [DWORD]
\                         ADDED CMOVCC, ALIASED JCC LOOPCC SETCC
\                         SIMPLIFIED CELL/ 8* 8/ 16*+ 16/MOD

\ GAH FRIDAY, DECEMBER 01 2006 - 22:18 ADDED FCMOVCC FCOMI ETC FFREEP INSTRUCTIONS
...
( NECESSARY BECAUSE OF ASM-HIDDEN IMMEDIATE )
: FORTH.IMMEDIATE IMMEDIATE ; \ a synonym \ 2023-10-25 ruv

( MISCELLANEOUS HOUSEKEEPING )
BASE @ DECIMAL ( SAVE THE BASE BECAUSE I HATE GRATUITOUS BASE BASHING )
\ : CELL- [ 1 CELLS ] LITERAL - ; \ IN KERNEL
HEX
: CELL/ 2 RSHIFT ;
: 8*    3 LSHIFT ;
: 8/    3 RSHIFT ;
...
: 8*+ 8* + ;
: 16*+  4 LSHIFT + ;
: 16/MOD DUP 0F AND SWAP 4 RSHIFT ;
HEX
: C0-8* 0C0 - 8* ;
: C0+ 0C0 + ;
: C0- 0C0 - ;
...
UNKNOWN   16BIT  INDIRECT      C4 ASM-OP   [WORD]
UNKNOWN   32BIT  INDIRECT      C4 ASM-OP  [DWORD]
...
: /PREFIX ['] _SAVE-INST IS SAVE-INST ;
: /POSTFIX ['] NOOP IS SAVE-INST ;
...
: FLAG-FOR-SIZE-PREFIX ( DO WE NEED A SIZE PREFIX? )
        ( SIZE -- FLAG )
        -IF DUP 8BIT - IF DEFAULT-SIZE - ELSE DROP 0 THEN THEN ;
...
: LIT-OP ( INSTERT THE LITERAL VALUE OF AN OPERAND INTO CODE )
        ( C:: -- )
        ( R:: -- X )
        ' >BODY DATA-@ POSTPONE LITERAL ; FORTH.IMMEDIATE
...
: [(E)BP]? ( DOES THE ADDRESS MODE HAVE EITHER [BP] OR [EBP] ALONE, OR [EBP] AS BASE? )
        ( -- FLAG ) \ ARM 02/12/2003 19:20:37 CORRECTION FOR <0 [EBP] [REG]>
        A16BIT? 6 MATCH-R/M? AND
        A32BIT? 5 MATCH-R/M? AND
        S-I-B DATA-@ 7 AND 5 = OR OR
        MOD-R/M DATA-@ 40 < AND ;
...
( BIT SCAN INSTRUCTIONS, CMOVCC INSTRUCTIONS )
: BS-COMPILE ( COMPILE A BIT SCAN INSTRUCTION, AND ALSO SELECTOR VALIDATION )
        ( PARAM -- | X \ PARAM -- )
        ?INST-PRE REGISTER GENERIC-ENTRY2 ?NOIMMED ?REG,R/M CODE-W,
        COMPILE-FIELDS ;
...
IN-ASM
  ...
  0BC0F '       BS-COMPILE OPCODE BSF
  0BD0F '       BS-COMPILE OPCODE BSR
  ...
  0400F '       BS-COMPILE OPCODE CMOVO
  0410F '       BS-COMPILE OPCODE CMOVNO
  0420F '       BS-COMPILE OPCODE CMOVNAE     : CMOVB  CMOVNAE ;   : CMOVC  CMOVNAE ;
  0430F '       BS-COMPILE OPCODE CMOVNC      : CMOVAE CMOVNC  ;   : CMOVNB CMOVNC  ;
  0440F '       BS-COMPILE OPCODE CMOVE       : CMOVZ  CMOVE   ;
  0450F '       BS-COMPILE OPCODE CMOVNE      : CMOVNZ CMOVNE  ;
  0460F '       BS-COMPILE OPCODE CMOVNA      : CMOVBE CMOVNA  ;
  0470F '       BS-COMPILE OPCODE CMOVNBE     : CMOVA  CMOVNBE ;
  0480F '       BS-COMPILE OPCODE CMOVS
  0490F '       BS-COMPILE OPCODE CMOVNS
  04A0F '       BS-COMPILE OPCODE CMOVP       : CMOVPE  CMOVP   ;
  04B0F '       BS-COMPILE OPCODE CMOVNP      : CMOVPO  CMOVNP  ;
  04C0F '       BS-COMPILE OPCODE CMOVNGE     : CMOVL   CMOVNGE ;
  04D0F '       BS-COMPILE OPCODE CMOVNL      : CMOVGE  CMOVNL  ;
  04E0F '       BS-COMPILE OPCODE CMOVNG      : CMOVLE  CMOVNG  ;
  04F0F '       BS-COMPILE OPCODE CMOVNLE     : CMOVG   CMOVNLE ;
    0B0 ' XCHG&ALU-COMPILE OPCODE CMPXCHG
...
0E2DB9B '            3BYTE OPCODE FCLEX
  0C0DA '    FMISC-COMPILE OPCODE FCMOVB
  0C8DA '    FMISC-COMPILE OPCODE FCMOVE
  0D0DA '    FMISC-COMPILE OPCODE FCMOVBE
  0D8DA '    FMISC-COMPILE OPCODE FCMOVU
  0C0DB '    FMISC-COMPILE OPCODE FCMOVNB
  0C8DB '    FMISC-COMPILE OPCODE FCMOVNE
  0D0DB '    FMISC-COMPILE OPCODE FCMOVNBE
  0D8DB '    FMISC-COMPILE OPCODE FCMOVNU
     02 '     FCOM-COMPILE OPCODE FCOM
  0F0DB '    FMISC-COMPILE OPCODE FCOMI
  0F0DF '    FMISC-COMPILE OPCODE FCOMIP
  ...
  0F6D9 '            2BYTE OPCODE FDECSTP
    ...
    0F0 '      FXP-COMPILE OPCODE FDIVRP    \ ADDED FOR COMPATIBILITY WITH SOME OTHER ASSEMBLERS
  0C0DD '    FMISC-COMPILE OPCODE FFREE
  0C0DF '    FMISC-COMPILE OPCODE FFREEP
...
0E3DB9B '            3BYTE OPCODE FINIT
  ...
  0FAD9 '            2BYTE OPCODE FSQRT
   ...
   0ED9 ' FBLD/STP-COMPILE OPCODE FSTENV
    ...
    0E0 '      FXP-COMPILE OPCODE FSUBRP    \ ADDED FOR COMPATIBILITY WITH SOME OTHER ASSEMBLERS
     05 ' FDIV/SUB-COMPILE OPCODE FSUBR
  0E4D9 '            2BYTE OPCODE FTST
  0E0DD '    FMISC-COMPILE OPCODE FUCOM
  0E8DB '    FMISC-COMPILE OPCODE FUCOMI
  0E8DF '    FMISC-COMPILE OPCODE FUCOMIP
  ...
  0F9D9 '            2BYTE OPCODE FYL2XP1
   ...
   080F '            2BYTE OPCODE INVD
    ...
    3CF '   SIZE-COND-COMP OPCODE IRETD
     77 '      JCC-COMPILE OPCODE JA     : JNBE   JA   ;
     73 '      JCC-COMPILE OPCODE JAE    : JNB    JAE  ; : JNC JAE  ;
     72 '      JCC-COMPILE OPCODE JB     : JNAE   JB   ; : JC  JB   ;
     76 '      JCC-COMPILE OPCODE JBE    : JNA    JBE  ;

\    0E3 '     JCXZ-COMPILE OPCODE JCXZ   : JECXZ  JCXZ ; \ это приводит к ошибке "A16:" в SEARCH !!!
    0E3 '     JCXZ-COMPILE OPCODE JCXZ
    0E3 '    JECXZ-COMPILE OPCODE JECXZ

     74 '      JCC-COMPILE OPCODE JE     : JZ     JE   ;
     7F '      JCC-COMPILE OPCODE JG     : JNLE   JG   ;
     7D '      JCC-COMPILE OPCODE JGE    : JNL    JGE  ;
     7C '      JCC-COMPILE OPCODE JL     : JNGE   JL   ;
     7E '      JCC-COMPILE OPCODE JLE    : JNG    JLE  ;
      0 '      JMP-COMPILE OPCODE JMP
     75 '      JCC-COMPILE OPCODE JNE    : JNZ JNE ;
     71 '      JCC-COMPILE OPCODE JNO
     7B '      JCC-COMPILE OPCODE JNP    : JPO JNP ;
     79 '      JCC-COMPILE OPCODE JNS
     70 '      JCC-COMPILE OPCODE JO
     7A '      JCC-COMPILE OPCODE JP     : JPE JP  ;
     78 '      JCC-COMPILE OPCODE JS
     9F '            1BYTE OPCODE LAHF
   020F '       BS-COMPILE OPCODE LAR
  ...
  0B20F '    LOAD-PTR-COMP OPCODE LSS
    ...
    0E1 '     LOOP-COMPILE OPCODE LOOPE  : LOOPZ   LOOPE  ;
    0E0 '     LOOP-COMPILE OPCODE LOOPNE : LOOPNZ  LOOPNE ;
  ...
  0AA0F '            2BYTE OPCODE RSM
    ...
    0AE '    WORD-STR-INST OPCODE SCASW
     97 '    SETCC-COMPILE OPCODE SETA   : SETNBE SETA  ;
     93 '    SETCC-COMPILE OPCODE SETAE  : SETNB  SETAE ; : SETNC  SETAE ;
     92 '    SETCC-COMPILE OPCODE SETB   : SETC   SETB  ; : SETNAE SETB  ;
     96 '    SETCC-COMPILE OPCODE SETBE  : SETNA  SETBE ;
     94 '    SETCC-COMPILE OPCODE SETE   : SETZ   SETE  ;
     9F '    SETCC-COMPILE OPCODE SETG   : SETNLE SETG  ;
     9D '    SETCC-COMPILE OPCODE SETGE  : SETNL  SETGE ;
     9C '    SETCC-COMPILE OPCODE SETL   : SETNGE SETL  ;
     9E '    SETCC-COMPILE OPCODE SETLE  : SETNG  SETLE ;
     95 '    SETCC-COMPILE OPCODE SETNE  : SETNZ  SETNE ;
     91 '    SETCC-COMPILE OPCODE SETNO
     9B '    SETCC-COMPILE OPCODE SETNP  : SETPO SETNP ;
     99 '    SETCC-COMPILE OPCODE SETNS
     90 '    SETCC-COMPILE OPCODE SETO
     9A '    SETCC-COMPILE OPCODE SETP   : SETPE SETP ;
...
: RESTORE-CURRENT ( RESTORE CURRENT TO ITS PREVIOUSLY SAVED VALUE )
        ( -- )
        CURRENT-SV DATA-@ ?DUP IF SET-CURRENT UNSAVE-CURRENT THEN ;
...
: _;CODE ( CREATE THE [;CODE] PART OF A LOW LEVEL DEFINING WORD )
        ?CSP !CSP POSTPONE (;CODE) POSTPONE [ INIT-ASM ;
...
: ENDM-IMM ( END A MACRO DEFINITION )
        POSTPONE LEAVE-MACRO POSTPONE ; FORTH.IMMEDIATE PREVIOUS RESTORE-CURRENT ;
    FORTH.IMMEDIATE
...
: EXIT ( REDEFINE EXIT TO TAKE CARE OF MACROS )
        ...
        ?LEAVE-MACRO EXIT THEN ; FORTH.IMMEDIATE
