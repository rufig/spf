...
: InsertNode                              ( r -- )
      ...
      Nil OVER lson !  Nil OVER rson !  match-length 0!
...
: Encode ( -- )
      ...
      0 text-buf  N F -  255  FILL

      ( Read F bytes into the last F bytes of the buffer. )

      DUP text-buf F infile READ-FILE THROW   ( s r count)
      ...
      BEGIN                                     ( s r)
            ...
            match-length @ Threshold > INVERT IF
      ...
      code-buf-ptr @ 1 > IF
            0 code-buf  code-buf-ptr @  outfile  WRITE-FILE THROW
            code-buf-ptr @ codesize +!
      THEN
;
...
: Decode                                  ( -- )
      ...
      BEGIN
            ...
            OVER 1 AND IF
                  ...
                  OVER text-buf C!              ( . r)
                  DUP text-buf 1 outfile WRITE-FILE THROW
            ...
            ELSE
                  ...
                  0 ?DO                         ( . r i)
                        ...
                        DUP 1 outfile WRITE-FILE THROW
...
\ S" eserv.exe" R/O OPEN-FILE THROW TO infile
\ S" pack.dat" W/O CREATE-FILE THROW TO outfile
\ Encode
