...
| `scan.f` | токенизатор поверхности spf4 (`\`, `( )`, строки `…"`, `{ локалы }`, `REQUIRE`, `\EOF`) с VECT-хуками + индекс определений по дереву `*.f` (FNV-хэш; обход каталогов — порт `~ac/lib/win/file/FINDFILE.F`) + cp1251→UTF-8 |
