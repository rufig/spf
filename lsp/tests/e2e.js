...
(async () => {
  ...
  // didOpen -> diagnostics: DU + FROBOZZ unknown, MYWORD/DUP/FIXTURE-HELPER known
  ...
  check(JSON.stringify(names) === JSON.stringify(['DU', 'FROBOZZ']), 'diag-names', names);
  const fro = diag1.params.diagnostics.find(d => d.message.includes('FROBOZZ'));
  check(fro && fro.range.start.line === 3 && fro.range.start.character === 15 && fro.range.end.character === 22, 'diag-range', fro && fro.range);

  // completion after "DU" (line 2, char 2)
  const comp = await request('textDocument/completion', { textDocument: { uri: mainUri }, position: { line: 2, character: 2 } });
  ...
  // hover on DUP (line 1, char 10)
  const hov = await request('textDocument/hover', { textDocument: { uri: mainUri }, position: { line: 1, character: 10 } });
  ...
  // hover on MYWORD (line 1, char 3)
  const hov2 = await request('textDocument/hover', { textDocument: { uri: mainUri }, position: { line: 1, character: 3 } });
  ...
  // hover on FIXTURE-HELPER (line 3, char 0) -> from the workspace index, with the stack comment line
  const hov3 = await request('textDocument/hover', { textDocument: { uri: mainUri }, position: { line: 3, character: 2 } });
  ...
  // definition of FIXTURE-HELPER
  const def = await request('textDocument/definition', { textDocument: { uri: mainUri }, position: { line: 3, character: 2 } });
  ...
  const def2 = await request('textDocument/definition', { textDocument: { uri: mainUri }, position: { line: 3, character: 24 } });
  check(def2.result && def2.result.uri === mainUri && def2.result.range.start.line === 1, 'definition-doc', def2.result);

  // definition of DUP -> spf-x64 seed source
  const def3 = await request('textDocument/definition', { textDocument: { uri: mainUri }, position: { line: 1, character: 10 } });
  ...
  // didChange: fix FROBOZZ -> diagnostics shrink to just DU
  notify('textDocument/didChange', {
    textDocument: { uri: mainUri, version: 2 },
    contentChanges: [{ text: MAIN_TEXT.replace('FROBOZZ', 'DUP') }],
  });
  const diag2 = await waitFor(m => m.method === 'textDocument/publishDiagnostics' && m !== diag1);
  check(diag2.params.diagnostics.length === 1 && diag2.params.diagnostics[0].message.includes('DU'), 'rediag', diag2.params.diagnostics);
