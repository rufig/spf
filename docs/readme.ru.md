...
SP-Forth является свободным программным обеспечением. См. раздел COPYRIGHT
...
*   Последнюю версию всегда можно найти на SF.net
    ...
    Вы можете получить исходники по [CVS](http://wiki.forth.org.ru/SpfCvs). 
...
*   Багтрекер (пожелания и багрепорты)
    ...
    Уведомления об активности на багтрекере идут в почтовую рассылку spf-tickets
...
условиями Стандартной Общественной Лицензии GNU, опубликованной Free Software Foundation.  
Для получения более подробной информации ознакомьтесь с [docs/license/gpl.ru.txt](license/gpl.ru.txt).
Все остальные файлы, включая код из `devel`, по умолчанию лицензированы под GNU LGPL, если не указано что-либо
другое.

Коротко говоря это значит следующее :

*    Вы не имеет права модифицировать и распостранять резальтат модификации ядра SPF не предоставляя при этом исходные коды.

*    Вы можете компилировать оригинальным SPF'ом и распостранять закрытое ПО.
