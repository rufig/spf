...
Это - краткое вступление в SPF для тех, кто уже знаком с Фортом.
...
inline-подстановку и peephole-оптимизацию. Подробнее на ForthWiki: "[Оптимизирующий компилятор][opt]".
...
В SPF есть стандартное слово для сравнения строк `COMPARE ( c1 u1 c2 u2 -- n )`, которое на
входе принимает две строки в формате "начало строки, её длина", а на выходе возвращает число,
*равное нулю при равенстве строк*, положительной единице, если первая строка предшествует
второй в лексикографическом упорядочении, и отрицательной единице - в обратном случае.

Есть также слово для сравнения строк без учёта регистра букв: `COMPARE-U`, подключаемое
строчкой: `~ac\lib\string\compare-u.f`.
...
или:
	...
	REQUIRE DLL NEW: ~ac/lib/ns/dll-xt.f
...
Подобного эффекта можно добиться и с помощью веторов, но так намного удобнее.
...
<style type="text/css" media="screen,projection,print">@import 'intro.md.css';</style>