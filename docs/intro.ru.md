...
Обработка исключений в SPF происходит согласно ANS-94 с ипользованием слов `THROW` и `CATCH`.
...
случае системных дополнительно печатается отчёт об исключении (как в режиме интерпретации).

[Scattered](#scatcoln) слово `AT-EXC-DUMP ( addr -- addr )` вызывается во время генерации
отчёта об исключении и даёт возможность выполнить дополнительные действия.
...
Коды ошибок, которые принимаются словом `THROW` и получаются на выходе `CATCH`,
интерпретируются согласно файлу spf.err, который находится в каталоге lib. Текстовые
сообщения которые печатаются в отчёте об исключении берутся из этого файоа.

Либа расширение `~ss/lib/except/try.f` кроме того что обходит обработку исключений 
уровня ОС, также обёртывает работу с исключениями в Delphi-подобный 
вид (см. примеры использования в `~ss/lib/except/try-example.f`).
