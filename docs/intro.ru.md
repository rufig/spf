...
* `TRUE TO ?C-JMP` включает оптимизацию хвостовой рекурсии (по-умолчанию выключена, иногда глючит)
...
временный динамический буфер, который остаётся неосвобожденным для последущего 
использования. При явном указании имени файла с помощью строчных литералов `S"`
и строчных бибиотек `~ac/lib/str*.f` необходимости в этом исправлении нет, 
...
Позволяют уже после определения слова добавлять в него новые действия.
	...
	..: INIT do2 ;.. 
	\ если здесь - то do2 и do1 именно в таком порядке
	..: INIT do3 ;.. 
	\ эквивалентно : INIT do2 do3 do1 ;
	\ и так далее
