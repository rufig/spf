...
Слова SPF и сопутствующих ему библиотек распределяются по разным словарям. 
Главные слова, слова взаимодействия лексикона задаются в главном словаре `FORTH`,
слова внутренние и/или те которые нуждаются в упрятывании -- в отдельных для 
каждого лексикона словарях (см. также <a href=#modules>модули</a>).

В тех случаях когда нужно вызвать внутреннее слово реализации из словаря/модуля
используется синтаксис вида `WORDLIST::word` (при этом должно быть понимание, что
лезть во внутренние детали лексиконов в обход предусмотренных автором слов 
взаимодействия -- это в общем-то, не очень хорошо). Пример:
...
Есть также случаи когда нужно разрешить неоднозначность выбора между несколькими
словами с одинаковым именем определённых в разных словарях. Точнее говоря 
неоднозначности-то нет, просто SPF может решать её не так, как хотелось бы нам. 
Пример:
...
нужно указывать в первом слове: `WORDLIST::' word`, `WORDLIST::POSTPONE word` и т.д.

<a id="catch"/>
###[Обработка исключений][start]

Обработка исключений в SPF происходит согласно ANS-94, через слова `CATCH` и `THROW`.

У нас есть (или специально формируется) слово в котором мы хотим поймать исключение,
берётся xt этого слова, и потом оно вызывается через `CATCH`:

	: r ... ;
	' r CATCH

или если мы хотим поймать исключение при вызове слова в другом определении (а это чаще
всего), то:

	: t ... ['] r CATCH ... ;

`CATCH` при успешной окончании работы слова `r` оставляет на стеке 0, при возникшем в
процессе выполнения `r` исключении -- оставит на стеке код ошибки.

Чтобы вызвать исключение самостоятельно, можно использовать `THROW`, на входе -- код
ошибки (если код равен нулю, то не расценивается как ошибка и выполнение продолжается
дальше):

	: r -1 THROW 1 2 3 ;
	' r CATCH ( -1 )

Также, `THROW` нужно писать после слов возвращающих код возврата ввода/вывода
(стековое обозначение -- `ior`). Это -- все операции с файлами (`CREATE-FILE`,
`OPEN-FILE`, `READ-FILE`, `WRITE-FILE`, `CLOSE-FILE` и т.д.) и операции с
памятью в куче (`ALLOCATE`, `FREE`, `RESIZE`).

Исключения уровня ОС (деление на ноль, ошибки доступа к памяти и т.д.)
по-умолчанию ловятся обработчиком `<EXC-DUMP>`, поэтому если написать:

	: r 1 0 / ;
	' r CATCH

то исключение не словится, и сработает обработчик исключений `<EXC-DUMP>`.
Чтобы обойти, это поведение заставив обрабатывать исключения ОС на общих
основаниях можно переопределить обработчик:

	' DROP TO <EXC-DUMP>

после чего `' r CATCH` будет вызывать не аварийный останов, а обработку с
выводом на стек кода возникшей ошибки.

Или, можно использовать `~ss/lib/except/try.f` который кроме того что
обходит обработку исключений уровня ОС, также обёртывает обработку исключений
в Delphi-подобный вид (см. примеры использования в `~ss/lib/except/try-example.f`).

Коды ошибок, которые посылаются слову `THROW` и получаются на выходе `CATCH`
интерпретируются согласно файлу spf.err, который может находится либо в папке SPF,
либо в папке lib. Этот файл является сборником текстовых сообщений присвоенных
номерам исключений.