...
: EARLIEST-NAME-IN ( wid -- nt|0 )
\ Вернуть самое раннее слово из списка wid.
\ Может вернуть текущее определение (незаконченное или скрытое).
  0 SWAP WID-HEAD@ BEGIN DUP WHILE NIP DUP NAME>NEXT-NAME REPEAT DROP
;
: CHAIN-WORDLIST ( wid.tail wid-empty -- ) \ wid-empty => wid
\ Если wid-empty не пуст, то инициировать исключение -12, иначе
\ добавить в wid-empty слова, которые доступны в wid.tail, сохраняя порядок.
