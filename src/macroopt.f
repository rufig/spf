...
внесен в версии 1.43 [2007-04-19 by mihail4444], но он НЕ дает эффекта, т.к. инструментальное
слово ":" принудительно устанавливает целевым список слов словаря TC-TRG.
Кроме того, решить эту проблему не достаточно. Слова, которые попадают в MACROOPT-WL,
далее будут не видны, т.к. этого словаря нет в контексте поиска.
В общем, это ограничение ЦК, который обеспечивает только единственный словарь в целевой системе.
2014-04-18 Сделан обход: все слова оптимизатора переносятся в его словарь уже после трансляции,
спец-средством в инструментальной системе.

Со списком NON-OPT-WL финт проходит, т.к. там слова создаются через CODE1
и ищутся они там только специально через слово "'" 
)
[THEN]
...
\  11XX.X000 1000.0011
   ...
   DUP          708B <> IF \ MOV  ESI, X [EAX]
   DUP          7589 <> IF \ MOV X [EAX], ESI
...
\ 110X.X0XX  1000.10X1
  DUP E4FC AND C088 <> IF \  MOV    E(ABCD)X , E(ABCD)X | (ABCD)L , (ABCD)L

\ 000X.X0XX  1000.10XX
  DUP E4FC AND 0088 <> IF \  MOV  [E(ABCD)X] , E(ABCD)X | (ABCD)(HL)
...
: DUP5B?      ( C -- C FLAG )
  ...
  OVER  0FD AND 0A0 = OR \  MOV EAX, X  | X , EAX
;
...
\ Проверка на не изменение  EAX
   ...
   CASE
     ...
     DUP 4888 <> IF \ MOV 1 [EAX] , CL
     DUP 558B <> IF \ MOV EDX , FC [EBP]
     DUP 4D8B <> IF \ MOV ECX , FC [EBP]
...
0=      IF DROP
  ...
  DUP  878D  =   \  8D873C100000      LEA     EAX , 103C [EDI]
        IF DROP FALSE EXIT THEN
  DUP  878B  =   \  8D873C100000      MOV     EAX , 103C [EDI]
        IF DROP FALSE EXIT THEN
...
TRUE ?~EAX !
  ...
  DUP 4503 <>       \ ADD     EAX , 0 [EBP]
        IF
  DUP 0523 <>       \ AND     EAX , 587744
        IF
  DUP 0503 <>       \ ADD     EAX , 587744
        IF
  DUP E0C1 <>       \ SHL     EAX , 7
        IF
...
0 ?~EAX !
  ...
  DUP 85048B <>     \ MOV     EAX , X [EAX*4]
        IF
  DUP 00B60F <>     \ MOVZX   EAX , BYTE PTR [EAX]
        IF
  DUP 80B60F <>     \ MOVZX   EAX , BYTE PTR X [EAX]
        IF
  DUP 40B60F <>     \ MOVZX   EAX , BYTE PTR X [EAX]
        IF
...
: ?EAX2EDX  ( OPX -   OPX' FALSE | FALSE TRUE | OPX' TRUE TRUE )
     ?OPlast     IF DROP FALSE TRUE EXIT THEN
  DUP @ :-SET U< IF DROP FALSE TRUE EXIT THEN
\ DROP FALSE TRUE EXIT
  DUP @ W@
  ...
  DUP 878B <>      \ MOV     EAX , 38 [EDI]
        IF
  DUP 418D <>      \ LEA     EAX , -1 [ECX]
        IF
  DUP C82B <>      \ SUB     ECX , EAX
        IF
  DUP 4589 <>      \ MOV     X [EBP] , EAX
        IF
  DUP 4D89 <>      \ MOV     0 [EBP] , ECX
        IF
  DUP 05C7 <>      \ MOV     X  , # Y
        IF
  DUP 87C7 <>      \ MOV     X [EDI] , # Y
        IF
  DUP 45C7 <>      \ MOV     X [EBP] , # Y
        IF
  FF AND  
  DUP A1   <>      \ MOV     EAX , X
        IF
...
: EAX2EDX0  ( OPX - OPX' FLAG )
  ...
  DUP 878B =       \ MOV     EAX , 38 [EDI]
        IF DROP
           978B OVER @  W!  CELL- TRUE EXIT
        THEN

  DUP 418D =       \ LEA     EAX , -1 [ECX]
        IF DROP
           518D OVER @  W!  CELL- TRUE EXIT
        THEN
  ...
  DUP A1   =      \ MOV     EAX , X
        IF DROP  DUP 1 OPresize
           158B OVER @  W!  CELL- TRUE EXIT
        THEN

HEX  U. DUP @ @ U.  U. ." EAX2EDX0" ABORT
;
...
: EAX2EDX  ( OPX - OPX' F | T )
  ...
  DUP 878B =       \ MOV     EAX , 38 [EDI]
        IF DROP
           978B OVER @  W!  CELL- FALSE EXIT
        THEN

  DUP 418D =       \ LEA     EAX , -1 [ECX]
        IF DROP
           518D OVER @  W!  CELL- FALSE EXIT
        THEN

  DUP C82B =       \   SUB     ECX , EAX
        IF DROP
           CA2B OVER @  W!  CELL- FALSE EXIT
        THEN

  DUP 4589 =       \ MOV     X [EBP] , EAX
        IF DROP
           5589 OVER @  W!  CELL- FALSE EXIT
        THEN
  ...
  DUP A1   =      \ MOV     EAX , X
        IF DROP  DUP 1 OPresize
           158B OVER @  W!  CELL- FALSE EXIT
        THEN

  U. DUP @ @ U.  U. ." EAX2EDX" ABORT
;
...
:  -EBPCLR   ( FLAG OPX  -- FLAG' )
  ...
  BEGIN
     ...
     DUP @ C@ FF   = IF DROP EXIT THEN \ CALL [x]
...
0=   IF  M\ C2 DTST
   ...
   IF
        :-SET OP1 @ U< IF   \ GOTO OP2 MOV     EAX , X [EBP]
...
\            MOV     EAX , FC [EBP]
 ...
 OP1  \ ." $"
      ...
      UNTIL
      IF  M\ D0 DTST
         OP1 ToOP0 -3 ALLOT FALSE   M\ D1 DTST
         EXIT
      THEN
...
\      OP0 @ W@ 458B XOR OR \ 567F65 8B45FC             MOV     EAX , FC [EBP]
   ...
   IF   M\ 6E DTST
	   DUP OPexcise CELL-
           BEGIN EAX2EDX0 UNTIL
           BEGIN EAX2EDX  UNTIL
           DROP
           DUP   C@  0C3 =
           IF    OP1 ToOP0           FALSE
           -2 ALLOT
           ELSE  D08B OP0 @ W! \ MOV     EDX , EAX
           THEN  M\ 6F DTST
           EXIT
    THEN  THEN 
...
OP2 @ W@ 878D XOR    \  8D8738000000      LEA     EAX , 38 [EDI]
OP1 @ W@ 1089 XOR OR \  8910              MOV     [EAX] , EDX
\ EAX=
0= IF  M\ 172 DTST
       9789 OP2 @ W!
...
DUP C@ C3 XOR
OP2 @ W@ D08B XOR OR \ 58AC79 8BD0              MOV     EDX , EAX
OP1 @ W@ 9789 XOR OR \ 58AC7B 899738000000      MOV     38 [EDI] , EDX
\ EAX=
0= IF  M\ 174 DTST
       OP2 OPexcise
       8789 OP1 @ W!
...
OP2 @ @ 1589D08B = \  8BD0           MOV     EDX , EAX
   ...
   IF  M\ E8 DTST
       ...
       DUP @ C3086D8D XOR   \ LEA     EBP , 8 [EBP]  RET
       OP0 @ W@ 458B XOR OR \ MOV     EAX , F8 [EBP]
0=     IF OP2 OPexcise
       THEN
       FALSE  M\ E9 DTST
...
OP2 @ @ 7FFFFF AND 08048D XOR   \ 8D0488        LEA     EAX , [EAX] [ECX*X]
OP1 @ W@ C7 XOR OR              \ C70055550500  MOV     [EAX] , # 55555
...
OP2 @ C@ B8 XOR    \ 59A227 B841000000  MOV     EAX , # 1000
OP1 @ C@ A3 XOR OR \ 59A22C A375995900  MOV     599975  ( Char1Glob+5  ) ,  EAX
0= 0 AND  IF   M\ 98 DTST
            OP2 @ 1+ @ OP1 @ 1+ @
            OP2 @ 2+ ! OP1 @ 1+ !
            05C7  OP2 @ W! \  MOV   599975 ,  # 1000
	       OP1 OPexcise
            FALSE M\ 99 DTST
            EXIT
    THEN
DUP C@  C3 XOR
OP2 @ W@ D18B XOR OR \  8BD1              MOV     EDX , ECX
OP1 @ W@ 1088 XOR OR \  8810              MOV     [EAX] , DL
OP0 @ W@ 458B XOR OR \  8B45FC            MOV     EAX , FC [EBP]
0=  IF   M\ 102 DTST
	       OP2 OPexcise
		0888  OP1 @ W! \ MOV     [EAX] , CL
            FALSE M\ 103 DTST
            EXIT
    THEN

DUP C@ C3 XOR
OP2 @ W@ 408D XOR OR \ 8D4001            LEA     EAX , 1 [EAX]
OP1 @ W@ 0888 XOR OR \ 8808               MOV     [EAX] , CL
OP0 @ W@ 458B XOR OR \ 8B45FC            MOV     EAX , FC [EBP]
0=  IF   M\ 106 DTST
		4888  OP2 @ W! \ MOV    1 [EAX] , CL
	       OP1 OPexcise
            FALSE M\ 107 DTST
            EXIT
    THEN

OP2 @ W@ 048B XOR    \  8B0424            MOV     EAX , [ESP]
OP1 @ W@ C88B XOR OR \  8BC8              MOV     ECX , EAX
OP0 @ W@ 458B XOR OR \  8B45F8            MOV     EAX , F8 [EBP]
0= IF  M\ 1A8 DTST
        0C8B OP2 @ W! \      MOV     ECX , [ESP]
       OP1 OPexcise
       FALSE  M\ 1A9 DTST
       EXIT
   THEN
...
OP2 @ 2+ C@
OP0 @ 2+ C@    XOR
OP2 @ W@ 45C7 XOR OR \ 57A6CA C745F800100000    MOV     F8 [EBP] , # 1000
OP1 @ W@ C8B XOR OR \ 57A6D1 8B0C24             MOV     ECX , [ESP]
OP0 @ W@ 458B XOR OR \ 57A6D4 8B45F8            MOV     EAX , F8 [EBP]
OP1 @ 2+ C@
OP0 @ 2+ C@    = OR
0= IF  M\ 1AA DTST
        B8 OP0 @ C! \      MOV     EAX , # X
...
0= IF  M\ 32A DTST
   ...
   THEN
M\ PPPP
   OP0 @ W@  E7FF AND 4589 =  \ MOV X [EBP] , EAX|EDX|EBX|ECX
...
OP1 @ 3 + @  XOR
OP0 @ @ FFFFFF AND 08048D XOR OR \  8D0401          LEA     EAX , [ECX] [EAX]
0= IF  M\ 15E DTST
       88048D  OP1 @ !
       OP1 ToOP0
       FALSE -7 ALLOT M\ 15F DTST
       EXIT
   THEN
...
DUP C@ C3 XOR
OP1 @ W@ 8F8D XOR OR \ 8D8F38000000      LEA     ECX , 38 [EDI]
OP0 @ W@ 01C7 XOR OR \ C70116000000      MOV     [ECX] , # 16
0=  IF  M\ 1EC DTST
         OP0 @ 2+ @ OP0 @  ! 
         87C7  OP1 @ W! \  MOV     38 [EDI] , # XX
         OP1 ToOP0
         -2 ALLOT
       FALSE  M\ 1ED DTST
       EXIT
...
OP1 @ C@   25 XOR  \ 579DE3 2555555555  AND     EAX , # 55555555
OP1 @ @ FFFFFF AND 00B60F <>  AND \ MOVZX   EAX , BYTE PTR [EAX] 
OR 
...
OP0 @  W@ ADD|XOR|OR|AND=
   IF
      ...
      IF   M\ AE DTST
         ...
         THEN  BEGIN EAX>ECX  UNTIL
           DROP
           OP0 @  W@  4503 =  \ ADD     EAX , X [EBP]
           IF   08048D OP0 @ !            ELSE   \ LEA EAX, [EAX+ECX]
...
OP0 @ 1- @ 08048DD9 XOR OR \    LEA     EAX , [ECX] [EAX]
0= IF  M\ 66 DTST
       OP2 OPexcise
...
OP0 @ @ FFFFFF AND 8048D XOR OR \ 58205B 8D0401         LEA     EAX , [ECX] [EAX]
0= IF  M\ 168 DTST
...
OP0 @ 2+ C@ XOR
OP2 @ W@ 4589 XOR OR \  8945FC            MOV     FC [EBP] , EAX
OP1 @ W@ C033 XOR OR \  33C0              XOR     EAX , EAX
OP0 @ W@ 4D8B XOR OR \  8B4DFC            MOV     ECX , FC [EBP]
0= IF  M\ 284 DTST
       C033 OP0 @ W! \ XOR     EAX , EAX
       C88B OP1 @ W! \ MOV     ECX , EAX
       FALSE -1 ALLOT  M\ 285 DTST
       EXIT
   THEN
...
\  DUP 0E3FF   = M_WL DROP 2+   REPEAT   \ JMP  EBX
  ...
  DUP 45C7  =   M_WL  DROP 7 +  REPEAT  \ MOV     X [EBP] , # Y
  DUP 87C7  =   M_WL  DROP A +  REPEAT  \ MOV     X [EBP] , # Y
  ...
  DUP   02048D = M_WL DROP 3 + REPEAT \ LEA  EAX, [EDX] [EAX]  \ a duplicate of this line removed below
  DUP   40B70F = M_WL DROP 4 + REPEAT \ MOVZX   EAX , WORD PTR FE [EAX] 
  DUP   40B60F = M_WL DROP 4 + REPEAT \ MOVZX   EAX , BYTE PTR XX [EAX]  \ 0FB640 reversed
...
\ 0000.0000 1011.1XXX 0000.1111
  ...
  DUP FFF8FF
  AND 18B00F = M_WL DROP 3 + REPEAT \ MOVZX  EBX, BYTE PTR [EAX]
  ...
  DUP 03448B = M_WL DROP 4 + REPEAT \ MOV    EAX, X [EBX] [EAX]
...
\ DUP 0E2FF = M_WL EVEN-EBP 2_,_STEP REPEAT  \ JMP  EDX
  ...
  DUP 87C7  = M_WL A_,_STEP  REPEAT  \ MOV     X [EDI] , # Y
  ...
  DUP 40B60F = M_WL 4_,_STEP      REPEAT \ MOVZX   EAX , BYTE PTR XX [EAX]  \ 0FB640 reversed
...
\ 0000.0000 1011.1XXX 0000.1111
  ...
  DUP FFF8FF
  AND 18B00F = M_WL 3_,_STEP      REPEAT \ MOVZX  EBX, BYTE PTR [EAX]
  ...
  DUP 03448B = M_WL 4_,_STEP      REPEAT \ MOV    EAX, X [EBX] [EAX]
...
\ ZZZZ IF ." z=" OP1 @ @ U. THEN
   ...
   OP0 @ W@ C083 =  \ ADD  EAX, # X
   IF
      OP0 @ 2+ C@ 01 =
      IF  M\ 406 DTST
       40 OP0 @ C! \  INC EAX
       FALSE  -2 ALLOT M\ 407 DTST
       EXIT
      THEN
      OP0 @ 2+ C@ FF =
      IF  M\ 406 DTST
...
0= IF M\ B4 DTST
   ...
   DUP 'DROP XOR
   OP0 @ C@ 25  XOR OR 0= \ TTTT AND \   AND  EAX, # X
   IF M\ 134 DTST
      A9 OP0 @ C!   \  TEST  EAX, # X
      FALSE M\ 135 DTST
      EXIT
